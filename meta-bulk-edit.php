...
class Storm_Meta_Bulk_Edit {
	...
	protected function init() {
		...
		add_filter( 'manage_'.esc_attr( $this->meta_settings['post_type'] ).'_posts_columns', array( $this, 'edit_meta_columns' ), 1 );
		add_action( 'manage_'.esc_attr( $this->meta_settings['post_type'] ).'_posts_custom_column', array( $this, 'manage_meta_columns' ), 10, 2 );
	...
	function edit_meta_columns( $columns ) {
		...
		$columns[$this->meta_settings['slug']] = esc_html( $this->meta_settings['title'] );
	...
	function storm_save_bulk_edit() {
		...
		if ( !empty( $post_ids ) && is_array( $post_ids ) && !empty( $meta_value ) ) {
			foreach( $post_ids as $post_id ) {
				update_post_meta( $post_id, esc_html( $this->meta_settings['slug'] ), sanitize_text_field( $meta_value ) );
			}
		}
	}
}
